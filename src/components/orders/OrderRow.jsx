...
const OrderRow = ({id, product, isDeleteOpen, isCompleted}) => {
  ...
  const getImageUrlFromProductId = productId => {
    ...
    let productfound = allProdData.products.filter(
  ...
  return (
    <HStack
      width={355}
      shadow={isDeleteOpen ? 5 : 0}
      key={id ? id : Math.random() * 12345}
      marginY={isDeleteOpen ? 5 : 3}
      padding={5}
      marginX={isDeleteOpen ? 4 : 0}
      bgColor={darkmode ? '#444444' : 'white'}
      borderRadius={25}>
      <Image
        width={110}
        height={110}
      ...
      <VStack marginLeft={3}>
        <HStack
          ...
          <Heading
            ...
            {product && product.productName
              ? truncate(product?.productName, 17)
              : ''}
          </Heading>
        </HStack>
        <HStack marginY={3} alignItems="center">
          <HStack
            ...
            width={5}
            height={5}
            borderRadius={100}></HStack>

          <Text color={darkmode ? 'white' : 'black'} marginLeft={2}>
            Color | Size = M
          </Text>
        </HStack>
        <HStack alignItems="center" justifyContent="space-between">
          ...
          <HStack
            bgColor={darkmode ? '#666666' : 'coolGray.800'}
            marginLeft={2}
            borderRadius={20}>
            <HStack paddingY={1} paddingX={2}>
              <Text color="white" fontWeight="semibold" fontSize={15}>
